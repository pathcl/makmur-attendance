...
function process_zk($machine, $data, $now){
	...
	foreach($data as $d){
		//$arr[] = $d[3]; 
		$arr[] = $d['time']; 
		//$arr[] = strlen($d[1]) < 4 ? substr('000' . $d[1], -4) : $d[1];
		$arr[] = strlen($d['user']) < 4 ? substr('000' . $d['user'], -4) : $d['user'];
		$arr[] = $machine;
		$arr[] = 1;
		//$arr[] = ($d[2] == 1 ? 'SignOut' : ($d[2] == 0 ? 'SignIn' : 'Auto'));
		$arr[] = ($d['status'] == 1 ? 'SignOut' : ($d['status'] == 0 ? 'SignIn' : 'Auto'));
		$arr[] = $now->format('Y-m-d H:i:s');
		$vals[] = '(?, ?, ?, ?, ?, ?)';
		$i++;
...
if ($row) {
	...
	if ($row['status'] == 1){
		...
		if ($row['relay']){
			...
			$resp = file_get_contents('http://' . $row['relay'] . '/attendance/pull.php?mch=' . $machine, false, $ctx);
			echo trim($resp);
			exit;
		}
		...
		if($errno = curl_errno($ch)){
			...
			if ($errno == 7){
				...
				if ($zk->connect()){
					...
					echo json_encode(array('success' => true, 'numrows' => $ctr, 'last_download' => $now->format('Y-m-d H:i:s')));
				} else {
